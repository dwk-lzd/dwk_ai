import {
    ...
    SpeechT5HifiGan // 语音合成模型 和音色结合 生成语音波形
...
// 单列模式 核心难点
// 多次执行tts ai 业务，但只是实例化一次
// 他的实例化开销太大，所以需要单列模式

class MyTextToSpeechPipeline {
    // AI 语音模型的数据源地址, 用于下载不同人说话的声音特征向量
    // 每个字,每个词
    ...
    static async getSpeakerEmbeddings(speaker_id) {
        ...
        // 张量
        // 下载文件  .bin
        // 数据的转换  将二进制数据转换为Float32Array
        // 创建一个张量，构建 1*512 维度的特征向量
...
self.onmessage = async (e) => {
    ...
    // 懒加载 llm 初始化和加载放到第一次任务调用之时，后续任务直接复用实例
    // 结构三个实例
